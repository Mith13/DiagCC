...
Vertex::Vertex(const Vertex & rhs)
{
	...
	this->index = 0;
	this->is_external=rhs.is_external;
}
...
void Vertex::setExternal(){
	is_external=true;
}

const int Vertex::getTimePosition() const
{
	if (t_pos >= 0)return t_pos;
	else return(aux::abortMessage("Time position was not set"));
}

const int Vertex::getUpLines() const
{
	return up;
}

const int Vertex::getDownLines() const
{
	return down;
}

const int Vertex::getIndex() const
{
	if (index >= 0)return index;
	else return(aux::abortMessage("Index was not set"));
}
...
{
	vertices_wrk.reserve(5);
	vertices.reserve(5);
	t_last = 0;
	up_max = 0;
	down_max = 0;
}
VertexPool::VertexPool(const VertexPool& pool) :vertices(pool.vertices)
{
	this->reset();
	this->t_last = pool.t_last;
	this->up_max = pool.up_max;
	this->down_max = pool.down_max;
}
VertexPool& VertexPool::operator=(const VertexPool& pool)
{
	this->vertices = pool.vertices;
	this->vertices_wrk = pool.vertices_wrk;
	this->t_last = pool.t_last;
	return *this;
}
void VertexPool::addVertex(const Vertex& v)
{
	int index = vertices.size();
	vertices.push_back(v);
	vertices[index].setIndex(index);
	if (v.getTimePosition() > t_last)t_last = v.getTimePosition();
	if (v.getUpLines() > up_max)up_max = v.getUpLines();
	if (v.getDownLines() > down_max)down_max = v.getDownLines();
	reset();
}

void VertexPool::getTimedEquivalentVertices(vector<vector<int>>& equivalent_vertices, const int t_pos, const int up_lines, const int down_lines)const
{
	vector<int> equal_vertices;
	for (auto& v : vertices)
	{
		if (v.getTimePosition() == t_pos && v.getUpLines() == up_lines && v.getDownLines() == down_lines) {
			equal_vertices.push_back(v.getIndex());
		}
	}
	if (equal_vertices.size() != 0)equivalent_vertices.push_back(equal_vertices);
}
void VertexPool::reset()
{
	vertices_wrk = vertices;
}
int VertexPool::allConnected()
{
	double sum = 0;
	for (auto& v : vertices_wrk)
	{
...
using std::map;;
using std::set;
using partition = std::vector<set<int>>;

//Custom comparator for PseudoEdge class
struct PseudoEdgeComparator
{
	bool operator ()(const graphs::PseudoEdge& lhs, const graphs::PseudoEdge& rhs) const noexcept {
...
void graphs::PrimitiveGraph::insertEdge(int a1, int a2)
{
	...
	if (a1 > getVertices() || a2 > getVertices() || a1 < 0 || a2 < 0) {
		std::string msg = "There is a problem inserting an edge (" + std::to_string(a1) + "," + std::to_string(a2) + ")";
		aux::abortMessage(msg);
	}
	a1--;
	a2--;
	...
	while (!colouring_wrk.empty()) {
		...
		for (int i = 0; i < colouring_out.size(); i++) {
			...
			if (split_set.size() > 1) {
				...
				map<PseudoEdge, set<int>, PseudoEdgeComparator> quasicolouring;
